...
{
    public class C8 : Graseby
    {
        
        public C8(AsyncClient client)
            : base(client)
        {
        ...
        {
            List<byte> debugBytes = CreatePumpAlarmPackageEx(m_PumpIndexs, 0x00000002, m_PumpCount);
            m_Client.Send(Common.Hex2Char(debugBytes.ToArray()));
        }
        ...
        {
            List<byte> debugBytes = CreatePumpAlarmPackageEx(m_PumpIndexs, 0x00010000, m_PumpCount);
            m_Client.Send(Common.Hex2Char(debugBytes.ToArray()));
        }
    }
}
